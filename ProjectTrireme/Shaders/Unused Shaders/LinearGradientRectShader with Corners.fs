...
void main()
{
	...
	float yRelativeToVector = clamp((rotatedPos.y - gradientOffset) / (gradientScale - gradientOffset), 0.0, 1.0);

	float distanceFromOriginAfterScales = min(xRelativeToVector, 1.0);
